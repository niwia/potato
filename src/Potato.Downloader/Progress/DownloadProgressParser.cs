...
/// <summary>
/// Parses raw stdout stream lines from DepotDownloaderMod, extracts progress percentages,
/// applies Exponential Moving Average (EMA) smoothing for speed calculations, and computes ETA.
/// </summary>
public sealed class DownloadProgressParser
{
    ...
    public DownloadProgressReport? ProcessLine(string? line, double? currentTimeSeconds = null)
    {
        ...
        // Check for percentage update in stdout
        ...
        if (match.Success && double.TryParse(match.Groups[1].Value, NumberStyles.Float, CultureInfo.InvariantCulture, out double depotPercentage))
        {
            ...
            var report = new DownloadProgressReport
            {
                ...
                FormattedEta = etaStr,
                DownloadedBytes = (ulong)Math.Min((double)TotalDownloadSize, totalProgressBytes),
                TotalBytes = TotalDownloadSize,
                IsValidating = false,
                RawLine = trimmed
            };
