...
namespace DepotDownloader
{
    ...
    static class ContentDownloader
    {
        ...
        private static int lastProgressTick = 0;

        public static void EmitProgress(uint depotId, ulong downloaded, ulong total, string currentFile = null, bool isValidating = false)
        {
            if (Config.JsonProgress)
            {
                double pct = total > 0 ? Math.Clamp((downloaded / (double)total) * 100.0, 0.0, 100.0) : 0.0;
                string fileEscaped = currentFile != null ? "\"" + currentFile.Replace("\\", "\\\\").Replace("\"", "\\\"") + "\"" : "null";
                Console.WriteLine($"{{\"type\":\"progress\",\"depotId\":{depotId},\"bytesDownloaded\":{downloaded},\"totalBytes\":{total},\"percentage\":{pct:F2},\"isValidating\":{(isValidating ? "true" : "false")},\"currentFile\":{fileEscaped}}}");
            }
        }
        ...
        private static async Task<DepotFilesData> ProcessDepotManifestAndFiles(CancellationTokenSource cts, DepotDownloadInfo depot, GlobalDownloadCounter downloadCounter)
        {
            ...
            {
                // Load the manifest from the provided file as the NEW/current manifest.
                // oldManifest is intentionally preserved from the configDir load above so it
                // can serve as the *previous* manifest for delta comparison on future runs.
                ...
                // Persist the manifest to configDir and write its .sha sidecar so that the
                // next invocation (with a different -manifestfile) can load it as oldManifest
                // via Util.LoadManifestFromFile() and perform a proper incremental delta instead
                // of re-downloading every file from scratch.
            ...
            else
            {
                ...
                if (oldManifestFile != null)
                {
                    ...
                    if (Config.VerifyAll || !hashMatches)
                    {
                        ...
                        {
                            if (Config.JsonProgress)
                                EmitProgress(depot.DepotId, depotDownloadCounter.sizeDownloaded, depotDownloadCounter.completeDownloadSize, currentFile: file.FileName, isValidating: true);
                            else
                ...
                if (neededChunks.Count == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        if (Config.JsonProgress)
                            EmitProgress(depot.DepotId, depotDownloadCounter.sizeDownloaded, depotDownloadCounter.completeDownloadSize, currentFile: file.FileName);
                        else
            ...
            DepotManifest.ChunkData chunk)
        {
            ...
            if (currentTick - lastProgressTick >= 500 || currentTick < lastProgressTick)
            {
                lastProgressTick = currentTick;
                if (Config.JsonProgress)
                {
                    EmitProgress(depot.DepotId, sizeDownloaded, depotDownloadCounter.completeDownloadSize);
                }
                else
                {
                    Console.WriteLine("{0,6:#00.00}% [Downloading...]", (sizeDownloaded / (float)depotDownloadCounter.completeDownloadSize) * 100.0f);
                }
            }
            ...
            if (remainingChunks == 0)
            {
                ...
                if (Config.JsonProgress)
                {
                    EmitProgress(depot.DepotId, sizeDownloaded, depotDownloadCounter.completeDownloadSize, currentFile: file.FileName);
                }
                else
                {
